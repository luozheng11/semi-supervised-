...
def test_single_volume_dual(image, label, net, num, writer, classes, patch_size=[256, 256]):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            out, out2 = net(input)
            out_net = torch.argmax(torch.softmax(out, dim=1), dim=1).squeeze(0)
          ...
          #   pred = zoom(out.squeeze(0), (x / patch_size[0], y / patch_size[2]), order=0)  # LITS
            pred = zoom(out, (x / patch_size[0], y / patch_size[1]), order=0)
            prediction[ind] = pred
        ...
        writer.add_image('valid/Prediction',out_show.unsqueeze(0) * 50, num+ind)
    metric_list = []
    #多类别的话取消下面的注释
